...
namespace ch11Homework_GH
{
    ...
    public class OrderService
    {
        ...
        //查询订单
        public static bool QueryOrder(out IEnumerable<Order> orders,string SearchBy, string info)
        {
            IEnumerable<Order> order = from o in OrderList where 1 == 0 select o;
            switch (SearchBy)
            {
                case "Order ID":
                    //可以用OrderBy来对订单进行排序
                    int infoID = int.Parse(info);
                    order = from o in OrderList
                                               where o.OrderID == infoID
                                               orderby o.OrderTotalMoney ascending
                                               select o;
                    break;
                case "Client Name":
                    order = from o in OrderList
                                               where o.ClientName == info
                                               orderby o.OrderTotalMoney ascending
                                               select o;
                    break;
                case "Client ID":
                    //可以用OrderBy来对订单进行排序
                    int ClientID = int.Parse(info);
                    order = from o in OrderList
                                               where o.OrderID == ClientID
                                               orderby o.OrderTotalMoney ascending
                                               select o;
                    break;
                default: 
        ...
        //修改订单
        public static bool UpdateOrder(int orderID,Order updateOrder)
        {
            IEnumerable<Order> orders = from o in OrderList
                          where o.OrderID == orderID
                          select o;
            if(orders.Count() > 0)//查询到有此订单
            {
                foreach (var o in OrderList)//找到OrderList中的此订单，进行修改
                {
                    if(o.OrderID == orderID)
                    {
                        o.ClientID = updateOrder.ClientID;
                        o.ClientName = updateOrder.ClientName;
                        o.orderItems = updateOrder.orderItems;
                        o.OrderTotalMoney = updateOrder.OrderTotalMoney;
                        o.client = updateOrder.client;
                    }
